...
@section('content')
    <div class="col-span-12 space-y-6 xl:col-span-12">
        <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
            ...
            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-title-sm font-semibold text-gray-800 dark:text-white/90">Customer Ledger</h2>
                    <p class="mt-0.5 text-theme-sm text-gray-500 dark:text-gray-400">Track sales, payments and
            ...
            <div id="panel-overview">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-5">
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white px-5 py-4 dark:border-gray-800 dark:bg-white/[0.03]">
                        <p class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">Total Sales</p>
                        <p class="mt-1.5 text-2xl font-semibold text-gray-800 dark:text-white/90">
                            Rs {{ number_format($globalTotals['total_sales'], 2) }}
                        </p>
                        <p class="mt-0.5 text-xs text-gray-400">Across {{ $globalTotals['count'] }} customers</p>
                    </div>

                    <div class="rounded-2xl border border-gray-200 bg-white px-5 py-4 dark:border-gray-800 dark:bg-white/[0.03]">
                        <p class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">Total Received</p>
                        <p class="mt-1.5 text-2xl font-semibold text-green-600">
                            Rs {{ number_format($globalTotals['total_paid'], 2) }}
                        </p>
                        <p class="mt-0.5 text-xs text-gray-400">Payments received</p>
                    </div>
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white px-5 py-4 dark:border-gray-800 dark:bg-white/[0.03]">
                        ...
                        <p class="mt-0.5 text-xs text-gray-400">Remaining to be collected</p>
                    </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div class="mb-4 flex flex-col gap-3 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                        <div>
                            <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">All Customers</h3>
                        ...
                        <form method="GET" action="{{ route('customers.ledger') }}">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <x-search-input placeholder="Search Customers here..." />
                                ...
                                @if (request('search') || request('status'))
                                    <a href="{{ route('customers.ledger', ['tab' => 'overview']) }}"
                        ...
                        <table class="min-w-full">
                            <thead class="border-y border-gray-100 dark:border-gray-800">
                                <tr>
                                    ...
                                    <th class="py-3 px-4 text-right text-theme-sm text-gray-500">Total Sales</th>
                                    <th class="py-3 px-4 text-right text-theme-sm text-gray-500">Total Received</th>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td class="py-3 px-4 text-right">
                                            @if ($customer['balance'] > 0)
                                                <span class="inline-flex items-center rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-medium text-red-600 dark:bg-red-500/10 dark:text-red-400">
                                                    Rs {{ number_format($customer['balance'], 2) }}
                                                </span>
                                            @else
                                                <span class="inline-flex items-center rounded-full bg-green-50 px-2.5 py-0.5 text-xs font-medium text-green-600 dark:bg-green-500/10 dark:text-green-400">
                                                    Cleared
                                                </span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="py-3 px-4 text-center">
                                            <button
                                                onclick="openStatement('{{ $customer['id'] }}', '{{ $customer['name'] }}')"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="py-16 text-center text-sm text-gray-400">No customers found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="border-t border-gray-200 px-6 py-4">
                        {{ $customers->appends(['tab' => 'overview', 'search' => request('search'), 'status' => request('status')])->links() }}
                    </div>
            ...
            {{-- TAB 2 — STATEMENT --}}
            @include('customer.ledgerPartial.statementTab')
    ...
    <script>
        ...
        function openStatement(customerId, customerName) {
