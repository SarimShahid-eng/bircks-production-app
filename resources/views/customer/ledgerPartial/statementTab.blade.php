<div id="panel-statement" class="hidden">
    ...
    {{-- FILTER BAR --}}
    <form method="GET" action="{{ route('customers.ledger') }}" id="statementForm">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if (request('customer_id') || request('from') || request('to'))
                        <a href="{{ route('customers.ledger', ['tab' => 'statement']) }}"
            ...
            <div class="custom-scrollbar max-w-full overflow-x-auto px-5 sm:px-6">
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        @foreach ($statement as $index => $entry)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                                ...
                                <td class="py-3 px-4">
                                    @if ($entry->type === 'Sale')
                                        <span class="inline-flex items-center rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-600 dark:bg-blue-500/10 dark:text-blue-400">
                                            Sale
                                        </span>
                                    @else
                                ...
                                <td class="py-3 px-4 text-right text-sm font-semibold {{ $runningBalance > 0 ? 'text-red-500' : 'text-green-600' }}">
                                    Rs {{ number_format(abs($runningBalance), 2) }}
                                    <span class="text-xs font-normal">{{ $runningBalance > 0 ? 'DR' : 'CR' }}</span>
                                </td>
                    ...
                    <tfoot class="border-t-2 border-gray-200 dark:border-gray-700">
                        <tr>
                            <td colspan="5" class="py-3 px-4 text-sm font-semibold text-gray-800 dark:text-white/90">Page Totals</td>
                            ...
                            <td class="py-3 px-4 text-right text-sm font-bold {{ $runningBalance > 0 ? 'text-red-500' : 'text-green-600' }}">
                                Rs {{ number_format(abs($runningBalance), 2) }}
                                <span class="text-xs font-normal">{{ $runningBalance > 0 ? 'DR' : 'CR' }}</span>
                            </td>
                        </tr>
        ...
        @else
            <div class="py-16 text-center">
                ...
                <p class="text-sm text-gray-400">
                    {{ request('customer_id') || request('from') || request('to') ? 'No transactions found for the selected filters.' : 'Select a customer or apply filters to view transactions.' }}
                </p>
            </div>
        @endif
