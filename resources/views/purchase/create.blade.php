...
@section('content')
    <div class="col-span-12 space-y-6 xl:col-span-12">
        <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
            ...
            <x-toast-error field="supplier_id" />
            <x-toast-error field="material_id" />

            {{-- PAGE HEADER --}}
            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-title-sm font-semibold text-gray-800 dark:text-white/90">Add Purchase</h2>
                    <p class="mt-0.5 text-theme-sm text-gray-500 dark:text-gray-400">Fill in the details to record a new
                        purchase</p>
                </div>
            ...
            <form method="POST" action="{{ route('purchases.store') }}">
                @csrf
                <input type="hidden" value="{{ @$purchase->id }}" name="updateId">
                <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">

                    {{-- Card Header --}}
                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Purchase Information</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Supplier, material, and pricing details for
                            this purchase.</p>
                    </div>
                    ...
                    <div class="border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                            ...
                            <div>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Material <span class="text-error-500">*</span>
                                </label>
                                <div class="relative">
                                    <select name="material_id" @class([
                                        'dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:text-white/90',
                                        'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700 dark:focus:border-error-800 pr-10' => $errors->has(
                                            'material_id'),
                                        'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:focus:border-brand-800 dark:bg-gray-900' => !$errors->has(
                                        ...
                                        @foreach ($materials as $material)
                                            <option value="{{ $material->id }}" @selected(old('material_id', $purchase->material_id ?? '') == $material->id)>
                                                {{ $material->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" name="quantity"
                                        value="{{ $purchase->quantity ?? old('quantity') }}" placeholder="e.g. 50"
                                        min="0.01" step="any" @class([
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" name="rate" value="{{ $purchase->rate ?? old('rate') }}"
                                        placeholder="e.g. 250.00" min="0" step="any"
                                        @class([
                                            'dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:text-white/90 dark:placeholder:text-white/30',
                                            'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700 dark:focus:border-error-800 pr-10' => $errors->has(
                                                'rate'),
                            ...
                            <div>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Total <span class="text-error-500">*</span>
                                </label>
                                <div class="relative">
                                    <input type="number" name="total" value="{{ $purchase->total ?? old('total') }}"
                            ...
                            <div>
                                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Purchase Date <span class="text-error-500">*</span>
                                </label>
                                <div class="relative">
                                    <input type="date" name="purchase_date"
                                        value="{{ old('purchase_date', isset($purchase->purchase_date) ? \Carbon\Carbon::parse($purchase->purchase_date)->format('Y-m-d') : '') }}"
                                        @class([
    ...
    {{-- Auto-calculate total from quantity × rate --}}
    <script>
