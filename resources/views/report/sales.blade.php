...
<div class="col-span-12 space-y-6 xl:col-span-12">
    <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-title-sm font-semibold text-gray-800 dark:text-white/90">Sale Report</h2>
                <p class="mt-0.5 text-theme-sm text-gray-500 dark:text-gray-400">All brick sale records with filters</p>
            </div>
        </div>
            ...
            <form method="GET" action="{{ route('reports.sales') }}">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    ...
                    <div class="flex items-end gap-2">
                        ...
                        @if(request()->anyFilled(['date_range', 'customer_id']))
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-sm text-gray-500 dark:text-gray-400">Total Bricks Sold</span>
                ...
                <p class="text-2xl font-bold text-gray-800 dark:text-white/90">{{ number_format($totalQuantity) }}</p>
                <p class="text-xs text-gray-400 mt-1">bricks</p>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-green-50 dark:bg-green-500/10">
                        <svg class="w-5 h-5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>
                        </svg>
                    </span>
                </div>
                <p class="text-2xl font-bold text-gray-800 dark:text-white/90">Rs {{ number_format($totalRevenue, 2) }}</p>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="px-5 py-4 sm:px-6">
                <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">Sale Details</h3>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($sales as $index => $sale)
                            <tr class="hover:bg-gray-50 transition-colors dark:hover:bg-white/[0.02]">
                                ...
                                <td class="py-3 px-4 text-sm text-gray-800 dark:text-white/90">{{ $sale->customer->name ?? '—' }}</td>
                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">{{ $sale->productionBatch->batch_no ?? '—' }}</td>
                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">
                                    {{ number_format($sale->quantity_sold) }}
                                    <span class="text-xs text-gray-400">bricks</span>
                                </td>
                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">Rs {{ number_format($sale->rate_per_brick, 2) }}</td>
                                <td class="py-3 px-4 text-sm font-medium text-green-600">Rs {{ number_format($sale->total, 2) }}</td>
                                <td class="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="py-16 text-center text-gray-500">No sales found</td>
                            </tr>
                        @endforelse
                    </tbody>
                    ...
                    @if($sales->count() > 0)
                        <tfoot>
                            <tr class="border-t-2 border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-white/[0.02]">
                                ...
                                <td class="py-3 px-4 text-sm font-bold text-gray-800 dark:text-white/90">{{ number_format($totalQuantity) }} bricks</td>
                                <td class="py-3 px-4"></td>
                                <td class="py-3 px-4 text-sm font-bold text-green-600">Rs {{ number_format($totalRevenue, 2) }}</td>
                                <td class="py-3 px-4"></td>
                            </tr>
                        </tfoot>
                    @endif
